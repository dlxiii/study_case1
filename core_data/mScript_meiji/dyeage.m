...
jj = 1;
for j = jj:1:24*30
    plot_VALUE(Num+k, [ncfile.lon, ncfile.lat], ncfile.nv, ncfile.(vartoplot{k})(:,j),...
        vartoplot{k}, ['layer No.',num2str(layer,'%02.2i')],...
        -1,...%minval,...
        1,...%maxval,...
        [ncfile.Times(j,1:10),' ',ncfile.Times(j,12:19)]);
...
% manually debug
zz=11;
water_mask = squeeze(ncfile.DYE(:,zz,:)<=1E-6);
water_age = squeeze(ncfile.DYE_AGE(:,zz,:)./ncfile.DYE(:,zz,:));
water_age(water_mask)=nan;
water_age=water_age/24/60;
fig = figure(02);
